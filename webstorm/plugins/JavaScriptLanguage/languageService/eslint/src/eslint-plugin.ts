...
export class ESLintPlugin implements LanguagePlugin {
        ...
        try {
            ...
            if (this.linter != null && this.options != null && this.CliEngine != null) {
                if (request.command === ESLintCommands.GetErrors) {
                    response.body = this.getErrors(request.arguments);
                } else if (request.command === ESLintCommands.FixErrors) {
                    response.body = this.fixErrors(request.arguments);
                }
            }
        } catch (e) {
    ...
    private getErrors(getErrorsArguments: GetErrorsArguments): string {
    ...
    private fixErrors(fixErrorsArguments: FixErrorsArguments): string {
