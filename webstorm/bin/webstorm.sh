...
# Try (in order): WEBIDE_JDK, webstorm.jdk, ../jre, JDK_HOME, JAVA_HOME, "java" in PATH.
# ---------------------------------------------------------------------
if [ -n "$WEBIDE_JDK" -a -x "$WEBIDE_JDK/bin/java" ]; then
  JDK="$WEBIDE_JDK"
elif [ -s "$HOME/.WebStorm2017.1/config/webstorm.jdk" ]; then
  JDK=`"$CAT" $HOME/.WebStorm2017.1/config/webstorm.jdk`
  if [ ! -d "$JDK" ]; then
    JDK="$IDE_HOME/$JDK"
  fi
...
VM_OPTIONS_FILE=""
if [ -n "$WEBIDE_VM_OPTIONS" -a -r "$WEBIDE_VM_OPTIONS" ]; then
  # explicit
  VM_OPTIONS_FILE="$WEBIDE_VM_OPTIONS"
elif [ -r "$HOME/.WebStorm2017.1/webstorm$BITS.vmoptions" ]; then
  # user-overridden
  VM_OPTIONS_FILE="$HOME/.WebStorm2017.1/webstorm$BITS.vmoptions"
elif [ -r "$IDE_BIN_HOME/webstorm$BITS.vmoptions" ]; then
  # default, standard installation
  VM_OPTIONS_FILE="$IDE_BIN_HOME/webstorm$BITS.vmoptions"
else
  # default, universal package
  test "$OS_TYPE" = "Darwin" && OS_SPECIFIC="mac" || OS_SPECIFIC="linux"
  VM_OPTIONS_FILE="$IDE_BIN_HOME/$OS_SPECIFIC/webstorm$BITS.vmoptions"
fi
...
if [ "$IS_EAP" = "true" ]; then
  ...
  if [ -r "$IDE_BIN_HOME/lib$AGENT_LIB.so" ]; then
    AGENT="-agentlib:$AGENT_LIB=disablealloc,delay=10000,probe_disable=*,sessionname=WebStorm2017.1"
  fi
fi
...
LD_LIBRARY_PATH="$IDE_BIN_HOME:$LD_LIBRARY_PATH" "$JAVA_BIN" \
  ...
  -Didea.paths.selector=WebStorm2017.1 \
